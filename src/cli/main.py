...
def search_command(args):
    ...
    if not playlist_dir and not transcript:
        print("❌ Please provide either -t (transcript file) or -p (playlist directory)")
        sys.exit(1)
    ...
    if playlist_dir:
        ...
        try:
            ...
            # Load and search playlist
            results = searcher.search_playlist(playlist_dir, args.query, args.results)
...
Whisper Models (faster → more accurate):
  ...
  turbo:  optimized large model, faster
    ...
    extract_parser.add_argument('--max-videos', type=int, help='For channel indexing: limit number of videos to index')
    ...
    # Search command
    search_parser = subparsers.add_parser('search', help='Search existing transcript or playlist')
    search_parser.add_argument('query', nargs='?', help='Search query')
    search_parser.add_argument('-t', '--transcript', help='Transcript file path')
    search_parser.add_argument('-p', '--playlist-dir', help='Playlist directory to search (searches all videos)')
    ...
    channel_parser = subparsers.add_parser('channel', help='Index and fetch videos from a YouTube channel')
    channel_subparsers = channel_parser.add_subparsers(dest='channel_command', help='Channel commands')

    channel_index_parser = channel_subparsers.add_parser('index', help='Index all videos from a channel')
    channel_index_parser.add_argument('source', help='YouTube channel URL (e.g., https://youtube.com/@handle/videos)')
    channel_index_parser.add_argument('-n', '--name', help='Output folder name (auto-generated if not provided)')
    channel_index_parser.add_argument('-o', '--output-dir', default='.', help='Output directory (default: current)')
    channel_index_parser.add_argument('--max-videos', type=int, help='Limit number of videos to index')
    channel_index_parser.set_defaults(func=channel_index_command)

    channel_fetch_parser = channel_subparsers.add_parser('fetch', help='Fetch transcripts for specific channel videos')
    channel_fetch_parser.add_argument('channel_dir', help='Channel extraction directory (created by channel index)')
    channel_fetch_parser.add_argument('videos', nargs='+', help='One or more YouTube video IDs or URLs')
    channel_fetch_parser.add_argument('-l', '--language', help='Subtitle/transcription language code (default: en)')
    channel_fetch_parser.add_argument('-w', '--whisper', action='store_true', help='Use Whisper for transcription')
    channel_fetch_parser.add_argument('-m', '--whisper-model', default='base', choices=['tiny', 'base', 'small', 'medium', 'large', 'turbo'], help='Whisper model size (default: base)')
    channel_fetch_parser.add_argument('--api', action='store_true', help='Use OpenAI API instead of local model (requires OPENAI_API_KEY)')
